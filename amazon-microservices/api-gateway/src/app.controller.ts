...
export class AppController {
  ...
  @Get('set-cache')
  setCache(@Query('key') key, @Query('value') value) {
    return this.appService.setCache(key, value);
  }

  @Get('get-cache')
  getCache(@Query('key') key) {
  ...
  async createOrder(@Body() payload) {
    ...
    try {
      ...
      const orderData = await lastValueFrom(
        this.order_service.send('create_order', payload),
      );

      if (orderData.error) {
        throw Error("Create order failed at order service")
      }
  ...
  async getElasticProducts() {
    // throw new Error("Get Elastic Product error!!!!")
    const data = await lastValueFrom(
      this.product_service.send('get_elastic_products', {}),
    );
  ...
  async createElasticProducts() {
    const data = await lastValueFrom(
      this.product_service.send('create_elastic_product', {}),
    );

    return data;
  }

  @Post('/elastic/products-auto')
  async createAutoElasticProducts() {
    const data = await lastValueFrom(
      this.product_service.send('create_auto_elastic_product', {}),
    );
